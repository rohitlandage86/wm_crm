...
const createCallLog = async (req, res) => {
  ...
  const calling_type = req.body.calling_type
    ...
    : null
  const call_time = req.body.call_time ? req.body.call_time : null
  const call_date = req.body.call_date ? req.body.call_date : null
  ...
  if (!mobile_number) {
    ...
    return error422('Call time is required.', res)
  } else if(!call_date) {
    return error422("Call date is required.", res);
  }
  ...
  //check 
  const isExistMobileAndCallDateQuery = 'SELECT * FROM call_logs WHERE mobile_number = ? AND call_date = ? AND call_time = ?';
  const isExistMobileAndCallDateResult = await pool.query(isExistMobileAndCallDateQuery, [mobile_number, call_date, call_time])
  if (isExistMobileAndCallDateResult[0].length >0) {
    return error422("Sorry this call log already exist", res)
  }
  ...
  try {
    ...
    const insertCallLogQuery = `INSERT INTO call_logs (mobile_number, customer_name, calling_type, call_time, call_date, call_duration, customer_id, untitled_id ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
...
const getAllCallLogs = async (req, res) => {
  ...
  try {
    let getCallLogQuery = ` SELECT c.*, e.name FROM call_logs c
        LEFT JOIN untitled u 
        ON u.untitled_id = c.untitled_id
        LEFT JOIN employee e 
        ON e.employee_id = u.employee_id
        WHERE c.customer_id = ${employeeDetails.customer_id}  `
    let countQuery = ` SELECT COUNT(*) AS total , e.name FROM call_logs c
        LEFT JOIN untitled u 
        ON u.untitled_id = c.untitled_id
        LEFT JOIN employee e 
        ON e.employee_id = u.employee_id
    ...
    if (employee_id) {
      getCallLogQuery += ` AND u.employee_id = '${employee_id}'`
      countQuery += ` AND u.employee_id = '${employee_id}'`
    }
    if (key) {
      ...
      if (lowercaseKey === 'activated') {
        ...
        getCallLogQuery += ` AND (c.mobile_number LIKE '%${lowercaseKey}%' OR LOWER(c.customer_name) LIKE '%${lowercaseKey}%'  OR LOWER(e.name) LIKE '%${lowercaseKey}%' ) `
        countQuery += ` AND (c.mobile_number LIKE '%${lowercaseKey}%' OR LOWER(c.customer_name) LIKE '%${lowercaseKey}%'  OR LOWER(e.name) LIKE '%${lowercaseKey}%' ) `
      }
    }
    if (fromDate && toDate) {
