...
const getSourceOfPatientWma = async (req, res, next) => {
    const untitled_id = req.companyData.untitled_id; 
    ...
    let sourceofpatientQuery = `SELECT s.*  FROM source_of_patient s LEFT JOIN untitled u ON u.untitled_id = s.untitled_id WHERE s.status = 1 AND u.category=2 AND s.untitled_id = ${untitledId} ORDER BY s.source_of_patient_name `;
    try {
